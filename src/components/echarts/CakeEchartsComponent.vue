<template>
  ...
  <EchartsComponentDef :chartsid="chartsid" :option="cakeEchart.option" :width="width" :height="height"
                       :sty="sty" :isDefault="isDefault" :isLegend="isLegend" ref="defEchartChild"
                       @legendChange="legendChange"></EchartsComponentDef>
</template>
<script>

  import EchartsComponentDef from '../echarts/EchartsComponentDef'
  import optionMB from '../echarts/js/cake-option.js' // 引入配置项
  export default {
    data() {
      return {
        cakeEchart: {
    ...
    methods: {
      ...
      updateHandleOption(paramOption, selectedItem) {
        if (paramOption != null && paramOption != undefined) {
          ...
          this.$refs.defEchartChild.updateHandleOption(this.cakeEchart.option, selectedItem)
        }
      },
      legendChange(obj) {
